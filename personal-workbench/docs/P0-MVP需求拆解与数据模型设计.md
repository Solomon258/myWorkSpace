...
| 编号 | 用户故事 | 验收标准 |
|---|---|---|
| US-2.1 | 作为用户，我点"整理"（或每天 16:30 自动触发），AI 把待整理条目分类 | 四类：任务 / 日程 / 收藏 / 知识；同时抽取标题、截止时间、优先级建议 |
...
| US-4.5.2 | 作为用户，我按关键词全文检索收藏 | 检索范围覆盖标题/内容/标签/链接；结果 ≤ 300ms |
| US-4.5.3 | 作为用户，我置顶高频查阅项（如班车表），归档失效项（如已买完） | 置顶项排在最前；归档默认隐藏、可筛选查看、可恢复 |
| US-4.5.4 | 作为用户，收集箱整理出的"收藏"类条目确认后进入收藏页签 | 携带原始内容与来源，可追溯 |
| US-4.5.5 | 作为用户，我可以修改任意收藏 | 可修改标题、正文、标签、链接和工作/生活分组；保存后更新全文检索内容并记录修改时间 |

界面约束：空间切换区采用“全部独占首行，工作/生活次行并列”；空间容器可使用晨雾蓝/玉簪青/暖纸珊瑚表达氛围，但记录卡统一使用中性白色载体，仅用分组徽标和标签色区分工作/生活，避免新增记录被大面积主题色染色。

### M4.6 记录互转（移至：任务 / 日程 / 收藏）

| 编号 | 用户故事 | 验收标准 |
|---|---|---|
| US-4.6.1 | 作为用户，我把记错菜单的记录移到另一个菜单（点「移至」→ 选「日程」或「收藏」） | 目标菜单立即出现该记录，当前菜单不再显示；每条记录都带「移至」入口，展开后是另外两个菜单 |
...
> 目标表没有的字段（任务的优先级、日程的时间、收藏的标签/链接）**必须逐项告知**：
...
- `inbox_item` 1 → 0..1 `task` / `schedule_event` / `favorite` / `knowledge_note`（整理产出）
...
| ai_category | VARCHAR(16) | task / schedule / favorite / knowledge，整理后写入 |
...
#### favorite（收藏，独立页签承载，支持全文检索）

定位：**时效性事务参考**（链接收藏、家庭采买、班车时刻等）。与"知识"的边界——知识是沉淀性内容、写入 Obsidian 进入 RAG 索引；收藏留在工作台速查，买完/过期即归档。
...
| 字段 | 类型 | 说明 |
|---|---|---|
| log_type | VARCHAR(16) | inbox / task / praise / favorite / pomo / plan |
...
3. 用户在"整理确认页"逐条确认/修改 → 生成 `task`/`schedule_event`/`favorite`/`knowledge_note` → `inbox_item.status=archived`。
