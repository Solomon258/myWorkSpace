...
class FavoriteFlowTest {
    ...
    private static final String TEST_ROOT = "target/test-workbench/favorite-" + UUID.randomUUID().toString();
    ...
    @Test
    void createsFavoriteWithAutoUrlTagsAndGroup() throws Exception {
        ...
        assertThat(jdbcTemplate.queryForObject(
                "SELECT COUNT(*) FROM activity_log WHERE log_type='favorite' AND category='work'", Integer.class)).isEqualTo(1);
        assertThat(jdbcTemplate.queryForObject(
                "SELECT COUNT(*) FROM activity_log WHERE log_type='favorite' AND category='life'", Integer.class)).isEqualTo(1);
    }

    /** 自动标题会落库成 favorite.title 的正式标题，不能带「…」省略号（历史上照搬了 demo 的 slice(0,24)+'…'）。 */
    @Test
    void autoTitleIsClippedWithoutEllipsis() throws Exception {
        String content = "这是一段超过二十四个字的长收藏内容用来验证自动标题的裁剪行为是否正确无误";
        ...
        assertThat(jdbcTemplate.queryForObject("SELECT title FROM favorite ORDER BY id DESC LIMIT 1", String.class))
                .isEqualTo(expected)
                .doesNotContain("…");
    }
    ...
    void explicitTitleIsUsedAsIsAndBlankFallsBackToContent() throws Exception {
        ...
        mockMvc.perform(post("/api/v1/favorites").session(session)
                        .contentType("application/json")
                        .content("{\"title\":null,\"content\":\"只有正文的收藏\"}"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.title").value("只有正文的收藏"));
        mockMvc.perform(post("/api/v1/favorites").session(session)
                        .contentType("application/json")
    ...
    void overlongTitleIsRejectedWithActionableMessage() throws Exception {
        ...
        mockMvc.perform(post("/api/v1/favorites").session(session)
                ...
                .andExpect(jsonPath("$.message").value("收藏标题不能超过 200 字"));
    }

    /**
     * 正文去掉必填（2026-09-12）：速查类收藏往往只有一行标题（「班车 7:20 小区门口」），
     * 强迫用户再往正文里抄一遍只会让人放弃记录。所以「只填标题」必须能存。
     */
    @Test
    void createsFavoriteWithTitleOnlyAndNoContent() throws Exception {
        mockMvc.perform(post("/api/v1/favorites").session(session)
                        .contentType("application/json")
     ...
     */
    @Test
    void rejectsFavoriteWithNeitherTitleNorContent() throws Exception {
        for (String body : new String[] {"{}", "{\"title\":\"\"}", "{\"content\":\"\"}",
            ...
            mockMvc.perform(post("/api/v1/favorites").session(session)
                    ...
                    .andExpect(status().isBadRequest())
                    .andExpect(jsonPath("$.code").value(1002))
                    .andExpect(jsonPath("$.message").value("收藏的标题和正文不能同时为空，至少写一项"));
        }
        assertThat(jdbcTemplate.queryForObject("SELECT COUNT(*) FROM favorite", Integer.class)).isEqualTo(0);
    }

    /** 编辑路径与创建路径必须是同一条规则：只守一个入口，另一个入口照样能存出空卡片。 */
    @Test
    void updateKeepsRuleThatAtLeastOneFieldIsFilled() throws Exception {
        long id = createFavorite("原标题", "原正文", null, "work");

        // 清空正文、标题还在 => 允许（正文是可空字段了）
        mockMvc.perform(patch("/api/v1/favorites/{id}", id).session(session)
        ...
        long second = createFavorite("待清空的标题", "只有正文的这一条收藏", null, "work");
        mockMvc.perform(patch("/api/v1/favorites/{id}", second).session(session)
                        .contentType("application/json").content("{\"title\":\"\"}"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.title").value("只有正文的这一条收藏"));
    ...
    void searchesByChineseKeywordUrlAndTagWithLiteralSpecials() throws Exception {
        createFavorite("公司班车时间表", "早班 7:50 软件园东门发车，晚班 18:30", null, null);
        createFavorite("报销 100% 完成", "报销流程已经 100% 走完", null, "work");
        createFavorite("字段 A_B 说明", "数据库 A_B 字段含义", null, "work");
        mockMvc.perform(get("/api/v1/favorites").param("q", "班车").session(session))
                .andExpect(status().isOk()).andExpect(jsonPath("$.data.length()").value(1));
        mockMvc.perform(get("/api/v1/favorites").param("q", "%").session(session))
                .andExpect(status().isOk()).andExpect(jsonPath("$.data.length()").value(1));
        mockMvc.perform(get("/api/v1/favorites").param("q", "_").session(session))
                .andExpect(status().isOk()).andExpect(jsonPath("$.data.length()").value(1));
    }

    @Test
    void pinSortsFirstAndArchiveRestores() throws Exception {
        long first = createFavorite("普通收藏", "普通内容", null, "work");
        long second = createFavorite("置顶收藏", "需要置顶的内容", null, "work");
        mockMvc.perform(post("/api/v1/favorites/{id}/pin", second).session(session)).andExpect(status().isOk());
        mockMvc.perform(get("/api/v1/favorites").session(session))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data[0].id").value((int) second))
                .andExpect(jsonPath("$.data[0].pinned").value(true));

        mockMvc.perform(post("/api/v1/favorites/{id}/archive", first).session(session))
                .andExpect(status().isOk()).andExpect(jsonPath("$.data.status").value("archived"));
        mockMvc.perform(get("/api/v1/favorites").session(session))
                .andExpect(status().isOk()).andExpect(jsonPath("$.data.length()").value(1));
        mockMvc.perform(get("/api/v1/favorites").param("archived", "true").session(session))
    ...
    @Test
    void updatesAndSoftDeletesFavorite() throws Exception {
        long id = createFavorite("原始标题", "原始内容", null, "life");
        mockMvc.perform(patch("/api/v1/favorites/{id}", id).session(session)
                        .contentType("application/json")
        ...
        mockMvc.perform(delete("/api/v1/favorites/{id}", id).session(session)).andExpect(status().isOk());
        mockMvc.perform(get("/api/v1/favorites").param("archived", "true").session(session))
                .andExpect(status().isOk()).andExpect(jsonPath("$.data.length()").value(0));
        assertThat(jdbcTemplate.queryForObject("SELECT deleted FROM favorite WHERE id=?", Integer.class, id)).isEqualTo(1);
    }
    ...
    void searchBenchmarkUnderTwoThousandRows() throws Exception {
        ...
        for (int i = 0; i < 2000; i++) {
            jdbcTemplate.update("INSERT INTO favorite(title,content,tags,grp,status,created_at,updated_at,is_demo) VALUES (?,?,?,?, 'active', ?, ?, 0)",
                    "批量收藏" + i, "内容" + i, "[\"批量\"]", i % 2 == 0 ? "work" : "life", now, now);
        }
        createFavorite("公司班车时间表", "早班 7:50 发车", null, "life");
    ...
    private long createFavorite(String title, String content, String tags, String grp) {
        String now = TimeUtil.now("Asia/Shanghai");
        jdbcTemplate.update("INSERT INTO favorite(title,content,tags,grp,status,created_at,updated_at,is_demo) VALUES (?,?,?,?, 'active', ?, ?, 0)",
                title, content, tags == null ? "[]" : tags, grp == null ? "work" : grp, now, now);
        return jdbcTemplate.queryForObject("SELECT id FROM favorite WHERE title=?", Long.class, title).longValue();
    }
}
