...
public class FavoriteCreateRequest {
     ...
     *
     * <p>去掉必填是因为「速查信息」这类收藏本来就只有一行标题：「班车 7:20 小区门口」
     * 填在标题里就够了，强迫用户再往正文里抄一遍只会让人放弃记录。
     * 但两者不能同时为空 —— 那会在列表里留下一张点不出任何内容的空卡片。
     * 这条「至少一项」是跨字段约束，注解表达不了，落在 {@code FavoriteService.requireTitleOrContent}。</p>
     */
    ...
    @Pattern(regexp = "auto|work|life", message = "收藏空间只能填 自动(auto) / 工作(work) / 生活(life)")
    private String grp = "auto";

    /**
     * 要挂到这条收藏上的附件 id（先在 {@code POST /api/v1/attachments} 上传拿到 id）。
